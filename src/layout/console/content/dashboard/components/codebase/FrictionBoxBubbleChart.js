...
export default class FrictionBoxBubbleChart extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (
      this.props.fileTableDto &&
      prevProps.fileTableDto !== this.props.fileTableDto
    ) {
    ...
    if (prevProps.hoverRowId !== this.props.hoverRowId) {
      this.updateCircleHover(
        prevProps.hoverRowId,
        this.props.hoverRowId
      );
    }
  }
  ...
  isBoxChange(prevProps, props) {
    return (
      props.boxTableDto &&
      prevProps.boxTableDto !== props.boxTableDto
    );
  }
  ...
  isModuleChange(prevProps, props) {
    return (
      props.moduleTableDto &&
      prevProps.moduleTableDto !== props.moduleTableDto
    );
  }
  ...
  displayFileChart(tableDto) {
    let treeData = this.createFileTreeRoot(
      tableDto.rowsOfPaddedCells
    );
  ...
  displayBoxChart(tableDto) {
    let treeData = this.createBoxTreeRoot(
      tableDto.rowsOfPaddedCells
    );
  ...
  displayModuleChart(tableDto) {
    let treeData = this.createModuleTreeRoot(
      tableDto.rowsOfPaddedCells
    );
  ...
  createChart(treeData) {
    ...
    this.height =
      ...
      titleMargin;
    this.width =
      DimensionController.getFullRightPanelHeight() - 10;
    let margin = 5;
    let padding = 2;
    ...
    this.circleGroup = this.createCircles(
      svg,
      packed,
      margin
    );

    this.labels = this.createCircleLabels(
      svg,
      packed,
      margin
    );
  ...
  createCircles(svg, packed, margin) {
    let xMinMaxConfusion = d3.extent(
      packed.children,
      function (d) {
    ...
    let parentOpacity = 0;
    if (
      this.props.drilldownBox ||
      this.props.drilldownModule
    ) {
      parentOpacity = 0.2;
    }
  ...
  createCircleLabels(svg, packed, margin) {
    ...
    labelGroup
      ...
      .attr(
        "clip-path",
        (d) => "url(#clip-" + d.data.name + ")"
      )
      .attr("text-anchor", "middle")
      .attr("alignment-baseline", (d) =>
        d.r > minRadius ? "top" : "middle"
      )
      .attr("class", "circleLabel")
      .attr("opacity", (d) => (d.children ? 0 : 1))
      .text((d) =>
        d.r > superMinRadius ? d.data.label : ""
      );

    labelGroup
      .selectAll(".circleMetric")
      .data(packed)
      .enter()
      .append("text")
      .attr("x", (d) => d.x + margin)
      ...
      .attr("opacity", (d) => (d.children ? 0 : 1))
      .text((d) =>
        d.r > minRadius ? d.data.friendlyValue : ""
      );
  ...
  createBoxTreeRoot(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      let confusionRate =
        confusion / (progress > 0 ? progress : 1);
      let child = {
        name: d[0].trim() + "-" + d[1].trim(),
        group: d[0].trim(),
        label: d[1].trim(),
        value: duration,
        friendlyValue:
  ...
  createModuleTreeRoot(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (duration > 0) {
        ...
        console.log(
          "filtering zero time row! " + d[0].trim()
        );
      }
    }
  ...
  createFileTreeRoot(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (duration > 0) {
        let child = {
          name: d[0].trim() + "-" + d[1].trim(),
          label: this.getFileName(d[1].trim()),
          value: duration,
          friendlyValue:
            UtilRenderer.convertSecondsToFriendlyDuration(
              duration
  ...
  render() {
    ...
    if (!this.props.moduleTableDto) {
      ...
      title = (
        <div id="chartTitle" className="chartTitle">
          Top Areas of Confusion
        </div>
      );
    ...
    return (
      <div>
        ...
        <div className="chartTitleWrapperWrapper">
          <div className="chartTitleWrapper">{title}</div>
        </div>
