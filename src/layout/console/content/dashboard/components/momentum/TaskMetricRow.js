...
export default class TaskMetricRow extends Component {
  ...
  getMomentumSquare(
    ...
    let cellSize = 25;
    let confusionFillHeight = Math.ceil(
      (confusionPercent / 100) * cellSize
    );
    let confusionPositionOffset = Math.floor(
      (1 - confusionPercent / 100) * cellSize
    );
    ...
    let svgContent = (
      ...
      >
        ...
        <rect
          x="0"
          y={confusionPositionOffset}
          width="30"
          height={confusionFillHeight}
          fill="#FF2C36"
        />
      </svg>
    );

    let popupContent = <div>Momentum: {momentum}</div>;
   ...
   */
  render() {
    let momentumSquare = this.getMomentumSquare(
      this.props.confusionPercent,
      this.props.momentumColor,
      this.props.momentum
    );
