...
export default class TagBubbleChart extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (
      this.props.wtfsTableDto &&
      prevProps.wtfsTableDto !== this.props.wtfsTableDto
    ) {
  ...
  isTagsChange(prevProps, props) {
    return (
      props.tagsTableDto &&
      prevProps.tagsTableDto !== props.tagsTableDto
    );
  }
  ...
  displayWtfsChart(tableDto) {
    let treeData = this.createWtfsTreeRoot(
  ...
  displayTagsChart(tableDto) {
    let treeData = this.createTagsTreeRoot(
      tableDto.rowsOfPaddedCells
    );
  ...
  createCircles(svg, packed, margin) {
    ...
    circleGroup
      ...
      .attr("fill", (d) =>
        d.children ? interp(1) : interp(0.8)
      )
      .attr("opacity", (d) =>
  ...
  createWtfsTreeRoot(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      let circuit = UtilRenderer.getCircuitName(
        d[1].trim()
      );
      let duration =
        UtilRenderer.getSecondsFromDurationString(
      ...
      backButton = (
        <div className="backButton">
          <Icon
            name={"backward"}
            size={"large"}
            onClick={this.props.zoomOutToTagsView}
          />
        </div>
      );
      title = (
