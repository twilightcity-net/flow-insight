...
export default class FervieSprite {
  ...
  draw(p5) {
    ...
    if (this.direction === FervieSprite.Direction.Up) {
      image = this.animationLoader.getAnimationImage(p5, AnimationId.Animation.FervieWalkUp, this.animationFrame, this.size);
      this.scaleAndDrawSprite(p5, image);
    } else if (this.direction === FervieSprite.Direction.Down) {
      image = this.animationLoader.getAnimationImage(p5, AnimationId.Animation.FervieWalkDown, this.animationFrame, this.size);
      this.scaleAndDrawSprite(p5, image);
    } else if (this.direction === FervieSprite.Direction.Right) {
      image = this.animationLoader.getAnimationImage(p5, AnimationId.Animation.FervieWalkRight, this.animationFrame, this.size);
      this.scaleAndDrawSprite(p5, image);
  ...
  scaleAndMirrorAndDrawSprite(p5, image) {
    p5.push();
    p5.translate(this.x + Math.round(this.size * this.scale) + Math.round(this.size/2 * (1 - this.scale)), this.y);
  ...
  scaleAndDrawSprite(p5, image) {
    p5.push();
    p5.translate(this.x + Math.round(this.size/2 * (1 - this.scale)), this.y);
    p5.scale(this.scale, this.scale);
    p5.image(image, 0, 0);
    p5.pop();
  }
  ...
  update(p5) {
    if (p5.keyIsDown(p5.LEFT_ARROW)) {
      this.changeDirection(FervieSprite.Direction.Left, -6, 0);
    } else if (p5.keyIsDown(p5.RIGHT_ARROW)) {
      this.changeDirection(FervieSprite.Direction.Right, 6, 0);
    } else if (p5.keyIsDown(p5.UP_ARROW)) {
      this.changeDirection(FervieSprite.Direction.Up, 0, -2);
      this.scale -= 0.01;
    }  else if (p5.keyIsDown(p5.DOWN_ARROW)) {
      this.changeDirection(FervieSprite.Direction.Down, 0, 2);
      this.scale += 0.01;
    } else {
