...
export default class CityEntrance extends Environment {
  ...
  static GROUND_IMAGE = "./assets/animation/city_entrance/city_entrance_background.png";
  static HOUSES_IMAGE = "./assets/animation/city_entrance/city_entrance_houses.png";
  static LAMPS_IMAGE = "./assets/animation/city_entrance/city_entrance_lamps.png";
  static SHADOW_IMAGE = "./assets/animation/city_entrance/city_entrance_shadow.png";

  static WALK_AREA_IMAGE = "./assets/animation/city_entrance/city_entrance_walkarea.png";
  static WALK_BEHIND_AREA_IMAGE = "./assets/animation/city_entrance/city_entrance_walkarea_behind.png";
  static WALK_BEHIND2_AREA_IMAGE = "./assets/animation/city_entrance/city_entrance_walkarea_behind2.png";
  ...
  getRightSpawnProperties() {
    ...
    return {x: Math.round((CityEntrance.IMAGE_WIDTH - 100) * this.scaleAmountX), y: Math.round((CityEntrance.IMAGE_HEIGHT - 120) * this.scaleAmountY), scale: 0.2};
  }
  ...
  drawBackground(p5, fervie) {
    let backgroundImage = this.animationLoader.getStaticImage(p5, CityEntrance.GROUND_IMAGE);
    let housesImage = this.animationLoader.getStaticImage(p5, CityEntrance.HOUSES_IMAGE);
    let lampsImage = this.animationLoader.getStaticImage(p5, CityEntrance.LAMPS_IMAGE);
    ...
    let walkBehindHouses = this.isWalkBehindHousesPosition(p5, fervie.getFervieFootX(), fervie.getFervieFootY());
    let walkBehindLamps = this.isWalkBehindLampsPosition(p5, fervie.getFervieFootX(), fervie.getFervieFootY());
  ...
  drawOverlay(p5, fervie) {
    let shadowOverlay = this.animationLoader.getStaticImage(p5, CityEntrance.SHADOW_IMAGE);
    let housesImage = this.animationLoader.getStaticImage(p5, CityEntrance.HOUSES_IMAGE);
    let lampsImage = this.animationLoader.getStaticImage(p5, CityEntrance.LAMPS_IMAGE);
    ...
    if (this.isWalkBehindHousesPosition(p5, fervie.getFervieFootX(), fervie.getFervieFootY())) {
      p5.image(housesImage, 0, 0);
      p5.image(lampsImage, 0, 0);
    }

    if (this.isWalkBehindLampsPosition(p5, fervie.getFervieFootX(), fervie.getFervieFootY())) {
      p5.image(lampsImage, 0, 0);
    }
